...
contract bigInt
{
    ...
    function shift(uint[2] x, int shft) constant returns(uint[2])
    {
        if (shft == 0)
            return x;
    ...
    function modSmall(uint[2] x, uint m) constant returns(uint)
    {
    ...
    function multModRercurse(uint[2] x, uint[2] y, uint[2] m) constant returns(uint[2])
    {
        // Check some special cases
        ...
        // Returns: (a * b/2) mod c
        uint[2] memory t = multModRercurse(x, rightShift(y) , m);
    ...
    function multModDouble(uint[2] x, uint[2] y, uint[2] m) constant returns(uint[2])
    {
        // Check some special cases
        if (compa(x, zero) == 0 || compa(y, zero) == 0) // One factor is zero
            return zero;
    ...
    // Multiplication
    function mul(uint[2] x, uint[2] y) constant returns(uint[4])
    {
